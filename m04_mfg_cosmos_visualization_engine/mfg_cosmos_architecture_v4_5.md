# MindForge — Architecture COSMOS Complète v4
...
- `mindforge_prd.md` v2.3 (§3.4 : 15 patterns résilience + §7.4 : 26 modes COSMOS + §15.5-15.6 : décisions COSMOS v3 + ARC)
- `mindforge_all_feature.md` (§5 : spécifications modes COSMOS v2)
...
| Hiérarchie profonde (≥4 niv.) | ❌ | ✅ **M10, M11** | Sunburst, Treemap |
| Taxonomie/Ontologie | ❌ | ✅ **M10, M11, M22** | Sunburst + Semantic Zoom |
...
| Relations bidirectionnelles | ❌ | ✅ **M12** | Chord Diagram |
...
Phase 1 — MVP+ (T0+3 mois, +5 modes → ~65% couverture)
  ...
  ├── M10 : Sunburst 🔴      (8j, G2 nouveau — 180KB)
  ├── M11 : Treemap 🟠       (6j, G2 réutilisé — 0KB)
...
Phase 2 — Post-MVP V1 (T0+6 mois, +5 modes → ~75% couverture)
  ...
  ├── M12 : Chord Diagram 🟡    (7j, nivo — 30KB)
...
// Trouver prérequis absents de mfg_concepts (user) pour les concepts actuels
...
//   Rejected  → fadeOut 300ms + stockage dans mfg_ai_rejections
...
**Vision** : Clic sur n'importe quel nœud → voir exactement D'OÙ vient ce concept → naviguer au passage précis dans MFG-READER SUITE.
...
// → Ouvre MFG-READER SUITE à la position exacte (page + paragraphe surligné)
...
// CREATE TABLE mfg_concept_provenance (
...
// CREATE TABLE mfg_ai_rejections (
...
export function renderPrivacyAwareHeatmap(
  data: HeatmapCell[],
  kThreshold: number = 100  // MindForge standard (vs k=3 RGPD minimum)
): HeatmapCell[] {
...
| **14** | **[NOUVEAU v4]** Tout nœud DOIT avoir au moins une entrée dans `mfg_concept_provenance`. Jamais de concept sans source traçable. | Intégrité des données |
...
- [ ] **D-SEC-002** : Rejets persistés dans `mfg_ai_rejections` (feedback loop)
...
Fusion de `cosmos-implementation-patterns.md` × `mindforge-prd-consolidé.md` × `cosmos-research.md`  
...
3. **Prioritization** : Product Manager intègre M9 (Concept Map), M10 (Sunburst), M14 (Radar) en priorité absolue Sprint 1
...
2. **Sprint 2 (M10 Sunburst)** : G2 integration, drill-down, breadcrumb navigation (8j)
3. **Sprint 3 (M14 Radar + M11 Treemap)** : Recharts Radar SMI 5D, G2 Treemap Squarify (11j)
...
1. **Sprint 5-6 (Modes Flux)** : Sankey/Alluvial (M13), Chord Diagram (M12), Circle Packing (M19) — 21j
...
- **Décision** : Contraindre la constellation de nœuds dans une enveloppe reproduisant le contour sagittal asymétrique d'un cerveau humain à l'aide de l'Équation de Sagittal de MindForge :
