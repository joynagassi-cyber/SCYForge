...
CREATE TABLE mfg_pivotiq_results (
  ...
  -- Synthèse
  unified_document_id UUID REFERENCES mfg_documents(id),
...
### 7.7 Module MFG-BRAIN — RAG Intelligent
...
- Moteur : MFG-BRAIN (triple retrieval + RRF + SSE streaming)
...
- ✅ Aucune inscription requise pour essayer MindForge
...
Étape 1 : Choix objectif (15 secondes)
  ...
  → Feedback immédiat : "Super ! 847 apprenants MindForge maîtrisent React"
...
Étape 4 : Premier succès (< 90 secondes)
  → MindForge génère instantanément :
...
→ Génère URL unique : mindforge.app/shared/{uuid}
...
**Schéma BDD :**
```sql
CREATE TABLE mfg_deck_shares (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  resource_type TEXT NOT NULL CHECK (resource_type IN ('deck','goal','document')),
  resource_id UUID NOT NULL,
  owner_id UUID NOT NULL REFERENCES mfg_users(id),
...
CREATE INDEX idx_deck_shares_token ON mfg_deck_shares(share_token);
```
...
→ User-generated content : 0 coût création contenu pour MindForge

Condition de lancement : >50 decks publics de qualité (seuil critique UX)
→ Phase 3 prototype côté équipe MindForge, Phase 4 lancement public
```
...
### 7.8 Module MFG-IMPRINT — Transfert Cognitif Hybride & L'Empreinte Vocabulaire
...
### 7.13 Module MFG-READER SUITE — Lecture Enrichie & Compréhension Immersive `[Rôle : Frontend & AI]`
...
```
MFG-READER SUITE — 4 Composants
├── FILE VIEWER          → Lire les fichiers sources ingérés (PDF, EPUB, MD, etc.)
├── WEB VIEWER           → Prévisualiser les fichiers GÉNÉRÉS par MindForge (read-only)
...
**Description** : Lecteur intégré MindForge pour tous les formats d'ingestion, enrichi par les résumés et visualisations IA. Ce n'est pas un aperçu — c'est un lecteur complet, fidèle, navigable.
...
- ✅ **Résumé chapitre** : Pré-généré à l'ingestion (stocké `mfg_chunks.summary`), mis à jour dynamiquement au changement de chapitre. Coût LLM : $0 (déjà calculé).
...
**Description** : Environnement de lecture immersive pour les fichiers générés par MindForge. Objectif : comprendre un document en profondeur en minimum de temps.
...
- ✅ **MFG-IMPRINT Inline** : Section IMPRINT dans panneau droit, Cran 5 pré-généré proposé après lecture section clé
...
**Description** : Agent IA qui détermine ce que l'utilisateur veut accomplir avec un document et orchestre les features MindForge les plus pertinentes. Une question → une expérience personnalisée.
...
pub fn select_cosmos_modes(doc: &DocumentAnalysis) -> Vec<CosmosMode> {
    let mut modes = vec![];
    // Structure hiérarchique → Sunburst (M10)
    if doc.structure == Hierarchical && doc.chapter_count > 5 { modes.push(M10); }
...
CREATE TABLE mfg_reader_sessions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  source_id UUID NOT NULL REFERENCES mfg_sources(id),
...
CREATE TABLE mfg_reader_annotations (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  source_id UUID NOT NULL REFERENCES mfg_sources(id),
  ...
  card_id UUID REFERENCES mfg_apex_cards(id),
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);

-- Historique Book Orchestrator
CREATE TABLE mfg_book_orchestrations (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  source_id UUID REFERENCES mfg_sources(id),
  document_id UUID REFERENCES mfg_documents(id),
...
CREATE TABLE mfg_reader_suite_sessions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  document_id UUID NOT NULL REFERENCES mfg_documents(id),
...
CREATE TABLE mfg_page_gallery_cache (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  document_id UUID NOT NULL REFERENCES mfg_documents(id),
...
-- Index Reader Suite
CREATE INDEX idx_reader_sessions_user ON mfg_reader_sessions(user_id, source_id);
CREATE INDEX idx_reader_annotations_source ON mfg_reader_annotations(user_id, source_id);
CREATE INDEX idx_book_orchestrations_user ON mfg_book_orchestrations(user_id, created_at DESC);
CREATE INDEX idx_reader_suite_sessions_user ON mfg_reader_suite_sessions(user_id, document_id);
CREATE INDEX idx_deck_shares_token ON mfg_deck_shares(share_token);
CREATE INDEX idx_deck_shares_public ON mfg_deck_shares(visibility, resource_type) WHERE visibility = 'public';
```
...
|---------|-------|--------|
| File Viewer adoption | >40% users ouvrent ≥1 fichier/mois | `mfg_reader_sessions` |
| Temps lecture moyen | >8 min/session | `time_spent_seconds` |
| Annotations créées | >2 annotations/session | `mfg_reader_annotations` |
| Book Orchestrator usage | >60% après ingestion | `mfg_book_orchestrations` |
...
CREATE TABLE mfg_concept_relations (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  source_concept_id UUID NOT NULL REFERENCES mfg_concepts(id),
  target_concept_id UUID NOT NULL REFERENCES mfg_concepts(id),
...
CREATE TABLE mfg_concept_communities (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  name TEXT,
  concept_ids UUID[],
  level INTEGER,
  parent_community_id UUID REFERENCES mfg_concept_communities(id),
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);

-- Cartes révision APEX
CREATE TABLE mfg_apex_cards (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  concept_id UUID REFERENCES mfg_concepts(id),
  document_id UUID REFERENCES mfg_documents(id),
...
CREATE TABLE mfg_apex_sessions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  started_at INTEGER NOT NULL,
...
CREATE TABLE mfg_apex_reviews (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  session_id UUID NOT NULL REFERENCES mfg_apex_sessions(id),
  card_id UUID NOT NULL REFERENCES mfg_apex_cards(id),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
...
-- ASCENT DAG Arêtes
CREATE TABLE mfg_ascent_edges (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  source_node_id UUID NOT NULL REFERENCES mfg_ascent_nodes(id),
  target_node_id UUID NOT NULL REFERENCES mfg_ascent_nodes(id),
...
CREATE TABLE mfg_ascent_exercises (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
