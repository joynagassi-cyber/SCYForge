...
CREATE TABLE mfg_imprint_registers (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  node_id UUID REFERENCES mfg_ascent_nodes(id), -- Nœud associé (v2)
  document_id UUID REFERENCES mfg_documents(id),
...
CREATE TABLE mfg_imprint_trees (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  register_id UUID NOT NULL REFERENCES mfg_imprint_registers(id),
...
CREATE TABLE mfg_shared_content_cache (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
...
CREATE TABLE mfg_agent_decisions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
...
CREATE TABLE mfg_drift_interventions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
...
CREATE TABLE mfg_proof_of_skill (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  certification_id TEXT UNIQUE NOT NULL, -- 'MFG-2026-06-08-REACT-84'
  ...
  pdf_url TEXT,
  linkedin_url TEXT,
  verification_url TEXT, -- mindforge.app/verify/XXX
...
CREATE TABLE mfg_routing_history (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  node_id UUID NOT NULL REFERENCES mfg_ascent_nodes(id),
...
CREATE TABLE mfg_user_gamification (
  user_id UUID PRIMARY KEY REFERENCES mfg_users(id),
...
CREATE TABLE mfg_gamification_events (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
...
CREATE TABLE mfg_llm_spend_log (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID REFERENCES mfg_users(id),
  goal_id UUID REFERENCES mfg_ascent_goals(id),
...
CREATE TABLE mfg_confidence_reports (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  document_id UUID REFERENCES mfg_documents(id),
  node_id UUID REFERENCES mfg_ascent_nodes(id),
...
CREATE TABLE mfg_chronicle_memory (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID UNIQUE NOT NULL REFERENCES mfg_users(id),
...
CREATE TABLE mfg_chronicle_disruptions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
...
CREATE TABLE mfg_chronicle_plans (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  disruption_id UUID REFERENCES mfg_chronicle_disruptions(id),
...
CREATE TABLE mfg_chronicle_conversations (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
...
CREATE TABLE mfg_arena_sessions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  node_id UUID REFERENCES mfg_ascent_nodes(id),
...
CREATE TABLE mfg_arena_personas (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
...
CREATE TABLE mfg_arena_skill_proof (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  ...
  included_in_pos UUID REFERENCES mfg_proof_of_skill(id),

  created_at INTEGER NOT NULL
);

-- Indexes CHRONICLE & ARENA
CREATE INDEX idx_chronicle_memory_user ON mfg_chronicle_memory(user_id);
CREATE INDEX idx_chronicle_disruptions_user ON mfg_chronicle_disruptions(user_id, created_at DESC);
CREATE INDEX idx_chronicle_plans_user ON mfg_chronicle_plans(user_id, goal_id);
CREATE INDEX idx_chronicle_conversations_user ON mfg_chronicle_conversations(user_id, created_at DESC);
CREATE INDEX idx_arena_sessions_user ON mfg_arena_sessions(user_id, goal_id, status);
CREATE INDEX idx_arena_sessions_domain ON mfg_arena_sessions(domain, difficulty_level);
CREATE INDEX idx_arena_personas_domain ON mfg_arena_personas(domain, difficulty_level);
CREATE INDEX idx_arena_skill_proof_user ON mfg_arena_skill_proof(user_id, goal_id);
```
...
CREATE INDEX idx_sources_user_status ON mfg_sources(user_id, status);
CREATE INDEX idx_sources_hash ON mfg_sources(content_hash); -- Déduplication
CREATE INDEX idx_documents_user ON mfg_documents(user_id, created_at DESC);
CREATE INDEX idx_concepts_user_smi ON mfg_concepts(user_id, smi_score DESC);
CREATE INDEX idx_cards_next_review ON mfg_apex_cards(user_id, next_review_at) WHERE deleted_at IS NULL;
CREATE INDEX idx_ascent_nodes_goal ON mfg_ascent_nodes(goal_id, status);
CREATE INDEX idx_agent_decisions_user ON mfg_agent_decisions(user_id, goal_id, created_at DESC);
CREATE INDEX idx_llm_spend_user ON mfg_llm_spend_log(user_id, created_at DESC);
CREATE INDEX idx_shared_cache_hash ON mfg_shared_content_cache(content_hash);
CREATE INDEX idx_shared_cache_domain ON mfg_shared_content_cache(goal_domain, reuse_count DESC);

-- Full-text search
CREATE INDEX idx_documents_fts ON mfg_documents USING gin(to_tsvector('english', content));
...
| **MindForge v2 optimisé** | **$0.006** | **$6/mois** |
...
- Dashboard coûts temps réel (`mfg_llm_spend_log`)
...
| Coût LLM/user/mois | <$0.10 Free, <$0.50 Premium | mfg_llm_spend_log |
...
│  mfg_ai_decisions (table de traçabilité)                      │
...
- MFG-IMPRINT (déclenchement auto par agent-04) → Phase 1 (déclenchement) / Phase 2 (UX complète)
...
| ID | Décision | Justification | Phase |
|----|---------|--------------|-------|
| D-OPT-009 | Sigmoïde de Vitalité Robust (ENGRAM) | Remplacer le modèle linéaire brut de vitalité par une équation sigmoïdale robuste amortie et écrêtée à $t = 60$ jours ($V_n(t)$). Cela évite le déclin trop agressif du savoir durant les premières semaines, tout en déclenchant l'archivage en dormance dans `mfg_engram_vault` de manière fiable à J90. | Phase 1 (Web) |
...
| D-OPT-026 | Offline-First Local Sync Queue | Enregistrer toutes les interactions et progressions FSRS locales dans la table `mfg_sync_queue` de l'IndexedDB locale du navigateur. Un service worker pousse asynchronement les lots à Insforge dès le retour du réseau. | Phase 1 (Web) |
...
| D-OPT-031 | Persistent IndexedDB WAL | Sauvegarder la file `mfg_sync_queue` locale sous forme de journal de transactions persistantes (Write-Ahead Log) dans l'IndexedDB, assurant la résilience et l'auto-réparation en cas de crash brutal du navigateur. | Phase 1 (Web) |
...
- **PAPER-038** : Genetic Programming — Treemaps & Sunbursts for Many-Objective Optimization (2018). Fondation M10/M11. Confiance 93%
...
| ASCENT-ORCHESTRATOR | Les 13 agents autonomes orchestrant l'ensemble de MindForge |
...
| Lecture fichiers | Téléchargement externe | **MFG-READER SUITE (File Viewer + Web Viewer + Reader Suite + Book Orchestrator + Page Gallery)** |
...
- [ ] SharedContentCache schema DB créé (mfg_shared_content_cache)
- [ ] mfg_llm_spend_log actif (monitoring dès J1)
...
- [ ] **D-SEC-002 Rejet** → Raccourcis V/X actifs + toast undo 10s + table `mfg_ai_rejections`
...
| **Mode Guest Onboarding** | Conversion trial→signup, impact funnel complet | Valider si MindForge veut permettre essai sans compte | 🟠 Haute |
...
**Document Finalisé Par** : Architecture Team MindForge  
**Date** : 2026-06-08  
**Version** : 2.5 (COSMOS v4 + 20 Patterns Résilience + 35 Compléments Analyse + MFG-READER SUITE + 19 Solutions COSMOS + 5 Opportunités Différenciation + CHRONICLE Agent-10 + ARENA Agent-11 + 10 Papers Rétention Science)  
