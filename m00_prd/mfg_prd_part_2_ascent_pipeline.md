...
Le `LEARNING-CONDUCTOR` synchronise dynamiquement l'état FSRS de l'apprenant (stabilité cognitive à court/moyen terme) avec la taxonomie de Bloom des nœuds d'apprentissage (Niveau 1 : Se souvenir, Niveau 2 : Comprendre, Niveau 3 : Appliquer, Niveau 4 : Analyser, Niveau 5 : Évaluer, Niveau 6 : Créer). 
...
4. **Métacognition ($C_n \in [0, 100]$)** : Évalue la justesse de l'auto-évaluation de l'apprenant pour contrecarrer l'effet Dunning-Kruger :
...
_(Source : TRIAD-Drop arXiv 2507.05285 — 89% accuracy, F1 0.88, validé sur 4 423 apprenants)_
...
Lorsqu'un apprenant décroche (absence > 7 jours), l'envoi de relances traditionnelles ou l'exposition brutale d'une pile énorme de révisions dues induit l'**effet d'évitement de l'échec** (le stress de la montagne de travail provoque l'abandon définitif).
...
- 🌉 **Cross-Domain Discovery** : "Votre concept React ressemble à [concept Python que vous connaissez]"
...
**Partage social :**
- LinkedIn : "J'ai maîtrisé React Hooks — SMI 87/100 (MindForge)"
...
- ID unique vérifiable (mindforge.app/verify/MFG-XXXX)
...
- Existe même quand l'utilisateur n'est PAS dans MindForge
...
**Mission** : Transformer la théorie en compétence réelle à travers des simulations haute-fidélité où l'IA incarne le monde réel — un acheteur, un patient, un manager, un public.

**Le problème résolu** : Pour la vente, le management, la médecine, la prise de parole — la théorie ne suffit pas. On ne peut pas prouver qu'on sait vendre avec un QCM. ARENA crée l'épreuve du feu avant la vraie.
...
ARENA-PERSONA   → Construit psychologie complète du personnage (Full-AI)
                            ↓
SESSION LIVE    → Roleplay haute-fidélité (vente, management, médecine...)
                            ↓
...
**Le Persona Full-AI** : ARENA analyse le contexte de l'user (domaine, niveau, techniques apprises) et construit un persona complet :
- Rôle et situation précise ("DSI PME tech, budget serré, déçu par un prestataire il y a 6 mois")
- Psychologie profonde (motivations, peurs, style de décision)
- Résistances calibrées selon le niveau de l'user (Flow Theory — ni trop facile, ni trop difficile)
- Triggers positifs (ce qui peut faire basculer la décision)
- État émotionnel dynamique (évolue authentiquement selon les réponses de l'user)
...
| **Communication Médicale** | Phase 2 | Annonce diagnostic, recueil historique, patient non-compliant |
| **Pédagogie & Formation** | Phase 2 | Expliquer à un débutant, animer groupe hétérogène |
| **Musique & Arts** (vision long-terme) | Phase 3+ | Improvisation, performance, critique |

**Le Debrief ARENA** — ce qui rend la session précieuse :
```
SCORE GLOBAL : 68/100
✅ Reformulation objection prix (min 1:23) — parfait
✅ Référence client similaire — a clairement rassuré
⚠️ Opportunité manquée (min 0:47) : "On a déjà un prestataire"
   → Aurais pu demander "Qu'est-ce qui vous ferait changer ?"
🔴 Monologue 45s sans question (min 1:05-1:50) → persona a décroché
   → Technique "Question toutes les 30 secondes" non appliquée

📚 Exercices générés automatiquement dans ASCENT :
   3 nouvelles cartes APEX sur "Écoute active en vente"
   + Exercice "Techniques de closing indirect"
```
...
PHASE 1 — SETUP (Jour 1, ~17 minutes automatiques)
  ...
  → [PREMIUM] Agent-10 CHRONICLE : Capture anchor behavior + planning hebdo J1
...
PHASE 2 — APPRENTISSAGE ACTIF (Jours 2 à N)
  ...
  → [PREMIUM] Agent-11 ARENA : Sessions pratique quand SMI ≥ 70 sur nœuds théoriques
...
**Boucle de décision :**
```
INPUT : "Générer le contenu du nœud React Hooks — useEffect"
           ↓
APEX-AGENT RAISONNE :
  - Type doc optimal ? → A06 (Cours magistral) pour nœud complexe
  - Ton optimal ? → T11 (Didactique) pour apprenants intermédiaires
...
```markdown
## 📊 Rapport de Confiance — "Introduction aux React Hooks"
...
| useState | 89 | 91% | 1 faible | ✅ |
| useEffect | 93 | 95% | 0 | ✅ |
| Règles Hooks | 88 | 88% | 2 faibles | ✅ |
| Exercices | 92 | 94% | 0 | ✅ |

**Sources** : 8 vidéos YouTube ✅ + 4 articles react.dev ✅
**Citations tracées** : 173/180 assertions liées à une source (96%)
**⚠️ Points attention** : 3 assertions support partiel (sections useState et Règles)
```
...
- ✅ **ARENA (Agent-11)** : Validation pratique par simulation Full-AI — vente, management, prise de parole, etc. + Proof of Skill "Conditions Réelles" [Premium]

**Infrastructure pipeline :**
- ✅ EventBus MindForgeEvent (communication inter-agents)
...
- **EPUB** : `epub 2.x` (déjà dans dépendances §6.1) — Livres numériques complets, chapitres + TOC + metadata Dublin Core. Découpage par chapitres pour MapReduce. Usage principal : persona Chercheur/Académique, manuels techniques.
...
- **Export .apkg** : Générer .apkg standard depuis cartes FSRS MindForge, permettre retour Anki, bundling médias (images/audio)
...
- BETA-1 : Taxonomiste (10 domaines : CS, Math, Physics, Bio, Business, Arts, Law, Medicine, History, Spiritual)
...
Pour garantir des certifications (Proof of Skill) d'une rigueur équivalente ou supérieure à des cursus certifiants (Coursera, Udemy, ECTS), les Knowledge Cards et examens d'un nœud subissent obligatoirement l'audit d'un sous-pipeline de 6 agents pédagogiques experts avant d'être débloqués pour l'étudiant :
...
3. **`QA-AGENT-03 : THE ALIGNMENT ORCHESTRATOR`** : S'assure que 100% du syllabus requis pour l'accréditation professionnelle (AWS, CFA, CEU) est couvert, déclenchant des recherches d'ingestion complémentaires si nécessaire.
...
MindForge intègre deux nouveaux agents de très haut niveau s'exécutant en arrière-plan :

* **`AGENT-15 : AXIOMATIZER` (L'Axiomatiseur de Connaissances)** :  
  Il résout le problème de l'inflation de micro-skills en s'exécutant de manière asynchrone côté serveur (Rust). Il capture les traces de réussite (`mfg_procedural_traces`), les abstrait, et les fusionne en une seule **Loi ou Méthode Fondamentale** unique (écrite dans `mfg_axioms` et partagée globalement de manière invisible). Une fois l'Axiome validé, il purge l'intégralité des micro-règles d'origine, éradiquant le cache.
...
MindForge structure l'orientation de l'apprentissage en deux parcours d'études distincts :
...
1. **Parcours A : L'Assimilation Active (Deep Understanding & Fast Competence)** :  
   ...
   - *La différence* : L'étudiant étudie un matériel pédagogique d'une qualité absolue, **mais aucun certificat officiel Proof of Skill n'est délivré à la fin**, ce qui préserve l'élite des diplômes de MindForge.
   - *Sûreté* : Un filtre d'intégrité minimal (anti-NaN, anti-division par zéro `softening_epsilon`) tourne de façon transparente en arrière-plan sans bloquer l'étudiant.  
   ...
   - *Sûreté* : Un filtre d'intégrité minimal (anti-NaN, anti-division par zéro) tourne de façon transparente en arrière-plan sans bloquer l'étudiant.
   - *Certification* : **ZÉRO CERTIFICAT DÉLIVRÉ**, préservant l'élite des diplômes MindForge.
2. **Parcours B : L'Accréditation Professionnelle (Accredited Certification)** :  
   ...
   - *Évaluation* : L'étudiant doit accomplir son parcours, réussir le SurveyJS exam et passer la session pratique de roleplay **ARENA** avec un **SMI de sortie $\ge 85/100$** pour générer sa Proof of Skill cryptée.
