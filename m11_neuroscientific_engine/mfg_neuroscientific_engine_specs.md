...
**Document ID** : SPEC-MFG-NEUROSCIENTIFIC-ENGINE-V1.0  
**Date** : 2026-06-12  
...
MindForge n'est pas un Second Brain passif. C'est un **système d'apprentissage actif cybernétique** aligné sur le fonctionnement biologique du cerveau. Ce document d'ingénierie détaille l'implémentation logicielle des 5 lois cognitives fondamentales validées empiriquement au cœur de la stack hybride MindForge.
...
### B. Spécification Technique MindForge
...
3. Il est transféré vers la table froide `mfg_engram_vault`.
...
### B. Spécification Technique MindForge
Lorsqu'un utilisateur réactive avec succès un nœud cible $N_i$ (par exemple en l'utilisant dans une session pratique ARENA ou en le citant dans ses notes), MindForge applique un **algorithme de suppression compétitive** à ses nœuds adjacents (1-hop) dans le graphe.
...
### B. Spécification Technique MindForge : Le "Mode FRICTION"
...
CREATE TABLE mfg_synaptic_vitality (
    ...
    node_id         UUID NOT NULL,        -- Référence polymorphique à mfg_ascent_nodes ou mfg_concepts
...
CREATE INDEX idx_vitality_user_score ON mfg_synaptic_vitality(user_id, vitality_score);
CREATE INDEX idx_engram_user_dormant ON mfg_engram_vault(user_id, dormant_since);
CREATE INDEX idx_forge_user_node ON mfg_forge_attempts(user_id, node_id);
```
...
En implémentant ces **5 spécifications neuroscientifiques directes**, MindForge s'arrache définitivement de la catégorie des clones simples de Second Brain. Le système devient une **extension vivante et plastique de la cognition de l'utilisateur**, le guidant par l'anti-fluidité vers une mémorisation indestructible, tout en gérant de manière saine, transparente et automatisée, le cycle naturel de la vie et de la mort de ses connaissances.
