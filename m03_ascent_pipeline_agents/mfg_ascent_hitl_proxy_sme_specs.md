...
**Document ID** : SPEC-MFG-AGENT16-HITL-V2.0  
**Date** : 2026-06-12  
...
L'agent ne cherche pas à valider ce qui est "assez bon". Son rôle est de garantir que tout contenu certifié par MindForge est **scientifiquement défendable devant un jury de pairs**.
...
* **SageMath Docker Sidecar** :  
  MindForge déploie en local (au sein d'un conteneur Docker sidecar léger sur Zeabur) **SageMath**, le géant open-source du calcul scientifique regroupant plus de 100 paquets spécialisés (SymPy, Maxima, PARI/GP, Singular). Le backend Rust communique avec lui via des requêtes HTTP/JSON-RPC ultra-rapides.
...
* **Philosophie** : Un certificat MindForge doit attester de manière absolue et indiscutable des compétences de l'utilisateur sur le marché mondial (ECTS, CEU).
* **Fonctionnement** :
