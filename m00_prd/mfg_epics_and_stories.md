...
inputDocuments: ["uploads/mindforge_prd.md", "uploads/mindforge_prd_neuro_consolidation_blueprint.md", "uploads/design.md", "uploads/experience.md", "uploads/mindforge_optimized_neuro_mathematics.md", "uploads/mindforge_brain_graph_rendering_specs.md"]
---

# MindForge v3.5 - Epic Breakdown

## Overview
Ce document fournit la décomposition complète des exigences et des décisions architecturales de MindForge v3.5 en Épiques (Epics) et récits utilisateurs (User Stories) prêts pour l'implémentation directe et sans hallucination par nos agents de développement.
...
* **NFR-RESIL-01** : Résilience réseau hors-ligne via une file `mfg_sync_queue` stockée en local dans l'IndexedDB sécurisée par un journal de transactions (WAL).
...
* **Critères d'Acceptation :**
  ...
  - **When** J'exécute le script DDL consolidé contenant `mfg_users`, `mfg_synaptic_vitality` et `mfg_project_deliverables`.
...
* **Critères d'Acceptation :**
  - **Given** Une liste de concepts issus de `mfg_synaptic_vitality`.
...
* **Critères d'Acceptation :**
  - **Given** Une file `mfg_sync_queue` contenant 50 révisions FSRS locales en attente de synchronisation.
