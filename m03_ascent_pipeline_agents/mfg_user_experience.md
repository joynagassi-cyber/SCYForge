...
# MindForge Experience Design

**Version :** 1.0.0  
**Date :** 2026-06-02  
**Objectif :** Définir l'architecture d'information, les comportements, les états, les interactions et les parcours utilisateurs de MindForge
...
MindForge s'articule autour de **deux routes majeures et distinctes** :
...
| ------- | ------ | ------------- | ------------------------ | --------------- |
| INGEST  | INGEST | URLs externes | mfg_sources + documents  | —               |
| LIBRARY | INGEST | Documents     | mfg_concepts + relations | INGEST          |
...
MindForge parle comme un **compagnon d'apprentissage expert mais accessible** :
...
MindForge intègre un assistant IA contextuel (BRAIN) inspiré de NotebookLM, accessible en permanence via un panneau latéral. Les composants suivants définissent les comportements et interactions du BRAIN panel.
...
**Protagoniste :** Marie, 24 ans, étudiante en informatique, découvre MindForge

**Objectif :** Comprendre MindForge en <60 secondes et ingérer sa première source

**Étapes :**

1. **Arrivée (0s)**
   - Marie ouvre MindForge pour la première fois
   - Splash screen 200ms
...
**Climax :** Marie réalise que MindForge a **automatiquement extrait des concepts** de sa vidéo et les a **connectés dans un graphe**. Elle comprend la valeur en voyant ses propres données.
...
## Les 5 Piliers Neuroscientifiques en Expérience Utilisateur (v3.0)

Pour rompre avec l'illusion d'assimilation des PKM classiques, MindForge injecte la biologie et la neuro-ergonomie directement dans l'expérience utilisateur :

### 1. L'ENGRAM Vault (Dormance Active)
- **L'expérience utilisateur** : Les nœuds inutilisés perdent progressivement en vitalité synaptique. Ils changent de couleur (du vert au gris slaty) puis disparaissent doucement dans le coffre-fort d'ENGRAMS (mémoire froide), assainissant l'interface pour éviter la fatigue visuelle.
- **Le défi de résurrection** : L'utilisateur ne peut pas réactiver un nœud d'un clic. Pour le ramener à la vie, il doit relever un **Reconstruction Challenge** : un Feynman challenge de rédaction libre évalué sémantiquement par l'APEX-AGENT en Rust (validation de similarité conceptuelle $\ge 70\%$).

### 2. Le Protocole FORGE (Génération obligatoire)
- **L'expérience utilisateur** : Le savoir n'est jamais présenté de façon passive. Lors de l'ouverture d'une Knowledge Card, le texte complet est flouté ou masqué. L'utilisateur fait face à une **amorce générative** (un texte à trous sémantique ou un défi de prédiction).
- **L'impact** : S'engager dans la génération d'une réponse *avant* de lire la solution triple le taux de mémorisation. La révélation subséquente (et l'affichage net de KaTeX) est vécue comme une récompense dopaminergique.

### 3. Le Mode FRICTION (L'Anti-Fluidité de Bjork)
- **L'expérience utilisateur** : L'illusion d'assimilation facile est éradiquée par l'anti-fluidité.
  - **Interleaving** : Les sessions de révisions mélangent activement 70% du domaine cible et 30% de domaines éloignés pour stimuler le cross-training cognitif.
  - **Désactivation de progression** : Les indicateurs de performance (barres d'XP) sont masqués en cours de session d'apprentissage active pour éviter de focaliser l'attention sur la récompense externe au détriment de l'effort synaptique.
  - **Reflection Delay** : Lors d'un quiz, un délai d'attente de 3 secondes verrouille le bouton de validation, forçant l'apprenant à s'arrêter et à faire face socratiquement au problème.

### 4. Le Graphe Morphogénétique Cérébral
- **L'expérience utilisateur** : L'utilisateur n'explore pas un nuage de points désordonné, mais voit son savoir prendre la forme d'un **cerveau humain en coupe sagittale** (COSMOS Mode 26).
- **Le ressenti** : Plus l'utilisateur mémorise, plus les zones correspondantes (cortex frontal pour la stratégie, cervelet pour le code) se colorent d'une émeraude vibrante consolidée, tandis que les zones froides ou obsolètes se teintent d'un gris glacial s'estompant doucement, donnant le sentiment d'un esprit cybernétique vivant.



### 5. La Console Créateur Élite (Dashboard Créateurs Formateurs)
Pour notre nouveau persona de **Créateur de Contenu Formateur (The Creator-Educator)**, l'expérience de pilotage de sa communauté est simplifiée et gamifiée :
- **Global Heat Map** : Visualisation globale de la cohorte d'étudiants sous forme de carte thermique de température. Le créateur voit en un coup d'œil quelles sections brillent d'une émeraude consolidée (maîtrise) et quelles sections sont "congelées" (zones d'abandon ou de blocage cognitif).
...
*Ce document est la référence contractuelle pour l'expérience utilisateur de MindForge. En cas de conflit entre ce document et tout wireframe, mock ou import, ce document prévaut. DESIGN.md prévaut sur les décisions visuelles, EXPERIENCE.md prévaut sur les comportements et parcours.*
