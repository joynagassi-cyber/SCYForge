...
**SCY Forge v3.0** rompt définitivement avec cette approche en devenant la première plateforme d'apprentissage **biologiquement cohérente et plastique**. Son architecture logicielle ne se limite pas à stocker de la donnée, elle imite, stimule et respecte activement le fonctionnement neurologique réel du cerveau humain :
...
#### Seuil de Dormance :
Si $V_n(t) < 20/100$, le concept est extrait du graphe actif et de l'index RAG, puis compressé dans `scy_engram_vault`.
...
CREATE TABLE scy_project_deliverables (
    ...
    confidence_score        INTEGER,       -- Score issu de scy_confidence_reports
    created_at              INTEGER NOT NULL,
...
CREATE TABLE scy_creator_cohorts (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
...
CREATE TABLE scy_creator_insights (
    id                  UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
...
CREATE TABLE scy_creator_clarifications (
    id                  UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
...
|--------------|-----------------------------------|---------------------|---------------------|
| **REQ-NEURO-01** | Oubli Actif : Les nœuds inutilisés entrent en dormance. | `scy_engram_vault` + Équation Vitalité | 🟢 **Intégré & Cohérent** |
...
## 🏁 Conclusion : L'Étoile du Nord de SCY Forge
...
CREATE TABLE scy_axioms (
    ...
    is_global       BOOLEAN DEFAULT true, -- Indique si partagé avec l'ensemble du réseau SCY Forge
    created_at      INTEGER NOT NULL,
    updated_at      INTEGER NOT NULL
);

-- Journal de synchronisation des Axiomes entre les cohortes/utilisateurs
CREATE TABLE scy_axiom_sync_logs (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    user_id         UUID NOT NULL REFERENCES scy_users(id) ON DELETE CASCADE,
    axiom_id        UUID NOT NULL REFERENCES scy_axioms(id) ON DELETE CASCADE,
    synced_at       INTEGER NOT NULL
);

-- Table des audits d'imitation d'experts humains (AGENT-16)
CREATE TABLE scy_hitl_proxy_audits (
    id                  UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
