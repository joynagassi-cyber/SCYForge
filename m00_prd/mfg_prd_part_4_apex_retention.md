...
CREATE TABLE mfg_teachback_sessions (
...
CREATE TABLE mfg_teachback_transcripts (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  session_id UUID NOT NULL REFERENCES mfg_teachback_sessions(id),
...
CREATE TABLE mfg_teachback_concept_patterns (
...
**Phase** : V1  
**Contexte** : En 2026, la majorité des professionnels utilisent l'IA au quotidien. Valider les compétences comme en 2015 (mémorisation pure, écriture de code à la main) n'a plus de sens dans de nombreux domaines. MindForge détecte le mode de travail réel de l'utilisateur et adapte ses critères de validation en conséquence.
...
CREATE TABLE mfg_user_work_mode (
...
CREATE TABLE mfg_domain_contexts (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  domain TEXT NOT NULL,
...
CREATE TABLE mfg_mode_adapted_exercises (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  original_exercise_id UUID REFERENCES mfg_ascent_exercises(id),
...
```
Implémentations dans MindForge :
...
```sql
-- Calcul automatique depuis mfg_apex_reviews (0 LLM)
SELECT 
...
*Note : En MindForge v2, ASCENT est piloté par la Pipeline Agentique (section 4). Les 8 modules deviennent des composants orchestrés par les agents.*
...
**Phase** : V2  
**Description** : Le Proof of Skill est la promesse centrale de MindForge — *"Si tu complètes ASCENT, tu as la compétence — vérifiable."* Ce n'est pas un simple certificat PDF. C'est un système multi-couches avec soumissions de preuves concrètes, évaluation multi-modale et vérification publique.
...
*La soumission d'une preuve concrète est obligatoire pour tout parcours de plus de 20h. C'est ce qui distingue MindForge d'un simple quiz de certification.*
...
║  🎯 [Titre du Parcours]                                      ║
║  📅 Certifié le [Date]                                       ║
║  🔑 ID : MFG-AAAA-MM-JJ-[DOMAINE]-[SCORE]                  ║
║                                                               ║
║  SCORE GLOBAL : [XX]/100                                     ║
║                                                               ║
║  DÉTAIL 5 DIMENSIONS :                                       ║
...
- ✅ **URL de vérification publique** : mindforge.app/verify/MFG-XXXX
  - Affiche score, date, compétences validées
  - Employeurs peuvent vérifier sans compte MindForge
...
CREATE TABLE mfg_skill_proof_submissions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  ...
  previous_submission_id UUID REFERENCES mfg_skill_proof_submissions(id),
...
CREATE TABLE mfg_skill_final_assessments (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  ...
  teachback_session_ids UUID[],       -- Liens vers mfg_teachback_sessions
...
CREATE TABLE mfg_proof_of_skill (
  ...
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID NOT NULL REFERENCES mfg_ascent_goals(id),
  submission_id UUID REFERENCES mfg_skill_proof_submissions(id),
  final_assessment_id UUID REFERENCES mfg_skill_final_assessments(id),
