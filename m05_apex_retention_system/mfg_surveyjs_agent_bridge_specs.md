...
Tu es le module de génération d'évaluations de MindForge. Ta mission est de générer un schéma JSON d'examen ou de quiz s'appuyant rigoureusement sur le format SurveyJS Form Library.
...
// Thème de Production Adaptatif (Dark/Light) hérité des variables CSS globales de MindForge
const applyCosmosThemeToSurvey = (survey: Model, theme: 'dark' | 'light') => {
  const css = survey.css;
  if (theme === 'dark') {
    css.root = 'mfg-survey-dark-root';
    css.question = {
      title: 'mfg-survey-dark-q-title',
      description: 'mfg-survey-dark-q-desc',
    };
  } else {
    css.root = 'mfg-survey-light-root';
  }
};
...
export const SurveyJSFormRunner: React.FC<SurveyJSFormRunnerProps> = ({
  ...
  const surveyModel = useMemo(() => {
    ...
    // Configuration des standards anti-anxiété de MindForge
  ...
  return (
    <div className="mfg-exam-workspace flex gap-6 w-full h-full p-6 bg-slate-950 text-slate-100 relative">
      {/* 🧭 NAVIGATION INDEX LATÉRAL (Le GPS de l'Examen) */}
      <div className="mfg-exam-gps-index w-72 flex flex-col gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] backdrop-blur-md">
      ...
      <div className="mfg-exam-runner-container flex-1 flex flex-col p-6 rounded-xl border border-white/5 bg-white/[0.01]">
