# 🧠 MINDFORGE — MFG-BRAIN, PROFESSOR AI & ONBOARDING ASCENT
...
**Score Confiance** : 95%
**Périmètre** : Formalise les fonctionnalités d'entrée du parcours ASCENT (Clarification + Starter Evaluator), le rôle transversal du **Professor AI**, le **MFG-BRAIN** (RAG + suggestions de nœud), et le module **BRAIN Advanced Chat Gen** (chat agentique mode normal / mode agentique).
**Compléments** : Étend `mindforge_prd.md §7.7`, `mindforge_all_feature.md §7`, `mindforge-ascent-explain.md §3 (Agent-01)`.
...
6. [MFG-BRAIN — Double Alimentation & Suggestions de Nœud](#6-mfg-brain)
...
Le parcours d'un apprenant dans MindForge démarre par une **séquence d'onboarding intelligente** qui garantit que le système comprend précisément *qui* est l'utilisateur et *ce qu'il veut* avant de produire le moindre contenu. Cette séquence évite la confusion en aval et conditionne la personnalisation pédagogique de tout le parcours.
...
└──────────────────────────────────────────────────────────────────────────────┘
   ...
   ║  • MFG-BRAIN : Q&A contextuel + 3 suggestions intelligentes/nœud      ║
   ╚════════════════════════════════════╤══════════════════════════════════╝
                                        ▼
                              ✅  COMPÉTENCE ATTEINTE
...
| Fonctionnalité décrite | Correspondance MindForge v2 | Statut |
|------------------------|------------------------------|--------|
...
| **Professor AI** | Couche d'accompagnement transversale au-dessus de **MFG-BRAIN** + **LEARNING-CONDUCTOR** | 🆕 Formalisé ici |
| **MFG-BRAIN (double alim. + suggestions)** | Extension de **§7.7 MFG-BRAIN** (RAG triple retrieval) | 🔄 Étendu |
...
MindForge expose **deux modes principaux** que l'utilisateur perçoit clairement dès l'entrée dans le produit.
...
Le **MDG-BRAIN** (MFG-BRAIN) est équipé du **Professor AI**, son **intelligence artificielle centrale** qui agit comme **guide et accompagnateur** de l'utilisateur tout au long de son parcours ASCENT.
...
│   • MFG-BRAIN (RAG triple retrieval + RRF)                     │
...
| **Moteur** | MFG-BRAIN (RAG triple retrieval + RRF + SSE streaming). |
...
```sql
-- ── CLARIFICATION ────────────────────────────────────────────────────────
CREATE TABLE mfg_clarification_sessions (
...
CREATE TABLE mfg_clarification_questions (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    session_id      UUID NOT NULL REFERENCES mfg_clarification_sessions(id),
...
-- ── STARTER EVALUATOR ────────────────────────────────────────────────────
CREATE TABLE mfg_starter_evaluations (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    user_id         UUID NOT NULL REFERENCES mfg_users(id),
    goal_id         UUID REFERENCES mfg_ascent_goals(id),
    ...
    dag_entry_node  UUID REFERENCES mfg_ascent_nodes(id), -- SINKT
    evaluation_data JSONB,                 -- détail des réponses + scoring IRT
    created_at      INTEGER NOT NULL
);

-- ── BRAIN — SUGGESTIONS PAR NŒUD ─────────────────────────────────────────
CREATE TABLE mfg_brain_suggestions (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    user_id         UUID NOT NULL REFERENCES mfg_users(id),
    node_id         UUID REFERENCES mfg_ascent_nodes(id),  -- nœud/contexte courant
...
CREATE TABLE mfg_brain_chat_sessions (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    user_id         UUID NOT NULL REFERENCES mfg_users(id),
...
CREATE TABLE mfg_brain_chat_deliverables (
    id              UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
    session_id      UUID NOT NULL REFERENCES mfg_brain_chat_sessions(id),
...
-- Index
CREATE INDEX idx_clar_sessions_user ON mfg_clarification_sessions(user_id, created_at DESC);
CREATE INDEX idx_starter_eval_user  ON mfg_starter_evaluations(user_id, created_at DESC);
CREATE INDEX idx_brain_sugg_node    ON mfg_brain_suggestions(node_id, created_at DESC);
CREATE INDEX idx_brain_chat_user    ON mfg_brain_chat_sessions(user_id, created_at DESC);
```
...
**Documents liés** : `mindforge_prd.md`, `mindforge_all_feature.md`, `mindforge-ascent-explain.md`, `mindforge_neurochain_explain.md`, `ANALYSE-COUTS-LLM-PIPELINE-AGENTIQUE.md`
