...
## Architecture Agentique Autonome : MindForge comme Machine à Compétences
...
**Objectif Absolu** : *"Si tu déclares un objectif dans MindForge, tu l'atteins — garanti par les agents."*  
...
1. [Vision Système — MindForge comme Organisme Vivant](#1-vision-système)
...
Pour que la pipeline à **13 agents autonomes d'ASCENT** fonctionne comme un organisme vivant cohérent et sans faille, MindForge unifie ses briques open-source d'élite au sein d'une **architecture de raccordement sémantique unique (Unified Agentic Pipeline)** :
...
Aujourd'hui, MindForge est une **collection de features excellentes** (Ingestion, NEURON-CHAINS, COSMOS, APEX, ASCENT, BRAIN, IMPRINT) — chacune étant forte individuellement, mais fonctionnant de façon relativement isolée, pilotée par l'utilisateur.

La vision est radicalement différente : **MindForge devient un organisme vivant** dont le seul but est de faire atteindre la compétence à l'utilisateur, en orchestrant automatiquement toutes ses features comme des organes au service d'un cerveau central.
...
### 1.3 Ce que MindForge Orchestre Automatiquement
...
impl GoalInterpreterAgent {
    pub async fn interpret(&self, raw_goal: &str, user_id: &Uuid) -> Result<LearningGoal> {
        ...
        // MindForge refuse de mentir à l'utilisateur en lui promettant une maîtrise d'expert en 3 mois.
        ...
        if scope.hours > 150.0 {
            return Err(AgentError::GoalTooLarge { 
                ...
                explanation: "Un parcours d'expert digne de ce nom ne se bâcle pas en quelques semaines. MindForge décompose votre grand objectif en plusieurs jalons de compétences ciblés et transparents pour vous garantir une véritable maîtrise.".to_string(),
            });
        }
...
impl SkillCertifierAgent {
    pub async fn certify(&self, goal_id: &Uuid, user_id: &Uuid) -> Result<ProofOfSkill> {
        ...
        Ok(ProofOfSkill {
            ...
            title: format!("{} — Certifié MindForge", dag.goal_title),
...
Voici comment chaque agent orchestre TOUTES les features de MindForge :
...
║  3. PDF Typst généré: "React Developer Certifié — MindForge"           ║
...
Pour l'évaluation finale (Gate 3) et la collecte de quiz sémantiques, MindForge s'appuie sur une architecture de rendu de formulaires dynamiques pilotée par schéma (JSON-driven) hautement optimisée en coûts :
...
║                                                                        ║
║  VÉRIFICATION:                                                        ║
║  🔗 mindforge.app/verify/MFG-2026-06-08-REACT-84                    ║
...
Pour donner une valeur d'embauche et une légitimité académique indiscutable à nos diplômes *Proof of Skill*, MindForge suit une feuille de route d'accréditation progressive en 3 Tiers (décrite en détail dans le document d'ingénierie `uploads/mindforge_accreditation_roadmap.md`) :
...
REDIRECTION SOURCES NON-EMBED :
  ...
  → "MindForge a extrait les points clés pour toi"
...
RÈGLE LÉGALE :
  Tous les screenshots sont GÉNÉRÉS par MindForge
...
┌─────────────────────────────────────────────────────────────────┐
│  DÉFINITION                                                     │
│  Parcours ASCENT complets construits de A à Z par MindForge.  │
...
```toml
[package]
name = "mindforge-ascent-pipeline"
version = "1.0.0"
edition = "2021"
...
│   ├── imprint_adapter.rs         # → MFG-IMPRINT
...
-- Index performance
CREATE INDEX idx_agent_decisions_user_goal ON mfg_agent_decisions(user_id, goal_id, created_at DESC);
CREATE INDEX idx_drift_interventions_user ON mfg_drift_interventions(user_id, created_at DESC);
CREATE INDEX idx_proof_of_skill_user ON mfg_proof_of_skill(user_id);
CREATE INDEX idx_routing_history_node ON mfg_routing_history(node_id, decided_at DESC);
```
...
**Priorité** : 🔴 CRITIQUE — C'est ce qui transforme MindForge en produit irremplaçable
