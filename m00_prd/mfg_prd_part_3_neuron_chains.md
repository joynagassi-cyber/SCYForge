...
| **M10** | **Sunburst Hiérarchique** 🔴 | Hiérarchie radiale concentrique (≥3 niveaux) | @antv/g2 v5 | ~180KB |
| **M11** | **Treemap Conceptuel** 🟠 | Partition rectangulaire hiérarchique | @antv/g2 v5 | ~0KB (via G2) |
...
| **M12** | **Chord Diagram** 🟡 | Relations bidirectionnelles circulaires | nivo/chord | ~30KB |
...
Principe : Traversal du graphe COSMOS sur les arêtes type "prerequisite_of"
           Identifier les nœuds prérequis absents de mfg_concepts (user)
...
- Validation/Rejet : raccourcis V/X + toast undo 10s + persistance `mfg_ai_rejections`
...
- Table `mfg_concept_provenance` : source_id, position (page/timestamp/paragraph), confidence
- Badge source sur nœud (zoom > 30%) : 🎥📄🌐✍️🤖
- Navigation one-click vers MFG-READER SUITE à la position exacte
...
Dans l'écosystème d'apprentissage de MindForge, les deux moteurs d'illustration générative opèrent en parfaite synergie, chacun couvrant un hémisphère distinct de la compréhension :
...
**Source** : audit-mfg-part1.md, lignes 227-234 | **Confiance** : 98%  
**Technologies** :
...
- ✅ Export LinkedIn/CV : Badge "SMI [score]/100 — [domaine] (MindForge)" + URL vérification
...
5. Création cartes MindForge avec états FSRS calculés
6. Import médias (images, audio) si présents

**Features import :**
- ✅ Préservation tags et decks hiérarchiques (mapping → tags MindForge)
...
#### 7.5.10 Export Anki — MindForge → .apkg
...
- ✅ Decks hiérarchiques (tags MindForge → sous-decks Anki)
- ✅ Tags préservés
...
- ✅ **Heat Map Annuelle** (Phase 2) :
  ...
  - Exportable en PNG (partage LinkedIn "Mon année d'apprentissage MindForge")
...
CREATE TABLE mfg_apex_cards (
  ...
  node_id UUID REFERENCES mfg_ascent_nodes(id),    -- Lien nœud ASCENT
...
CREATE TABLE mfg_apex_sessions (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
  goal_id UUID REFERENCES mfg_ascent_goals(id),    -- Contexte ASCENT si applicable
  node_id UUID REFERENCES mfg_ascent_nodes(id),    -- Nœud ciblé si micro-session
...
CREATE TABLE mfg_apex_mirror_responses (
  id UUID PRIMARY KEY DEFAULT gen_uuid_v7(),
  session_id UUID NOT NULL REFERENCES mfg_apex_sessions(id),
  user_id UUID NOT NULL REFERENCES mfg_users(id),
...
CREATE INDEX idx_cards_next_review ON mfg_apex_cards(user_id, next_review_at)
  WHERE deleted_at IS NULL AND is_suspended = false AND is_buried = false;
CREATE INDEX idx_cards_leech ON mfg_apex_cards(user_id, is_leech)
  WHERE is_leech = true;
CREATE INDEX idx_cards_node ON mfg_apex_cards(node_id)
  WHERE node_id IS NOT NULL;
CREATE INDEX idx_reviews_card_time ON mfg_apex_reviews(card_id, reviewed_at DESC);
CREATE INDEX idx_sessions_user ON mfg_apex_sessions(user_id, started_at DESC);
CREATE INDEX idx_mirror_user ON mfg_apex_mirror_responses(user_id, responded_at DESC);
```
