...
| Hiérarchie / taxonomie | **Sunburst (M10)** / **Treemap (M11)** | Arborescence d'API |
...
Dans l'écosystème d'apprentissage de MindForge, ces deux moteurs d'illustration générative opèrent en parfaite synergie, chacun couvrant un hémisphère distinct de la compréhension :
...
**Table** `mfg_content_timestamps` : `content_hash · concept_id · timestamp_start · timestamp_end · relevance_score (0.0–1.0)`.
...
Pour éviter d'avoir à coder manuellement des dizaines d'interfaces d'examens, d'outils d'administration pour les enseignants, de tableaux de bord statistiques ou de moteurs de génération de PDF, MindForge intègre la suite complète **SurveyJS Suite** (Form Library, Survey Creator, Dashboard, PDF Generator). 
...
#### A. Les 4 Piliers de la Suite SurveyJS & Leurs Rôles dans MindForge :
...
- Toutes les données de soumission d'examen sont stockées de manière sécurisée en base cloud (PostgreSQL, table `mfg_skill_final_assessments`).
...
PILIER 3 — Bibliothèque visuelle versionnée (table mfg_software_assets)
```
...
> **Règle légale (A.6)** : tous les screenshots sont **générés** par MindForge (jamais scrapés) → propriété intellectuelle propre.
