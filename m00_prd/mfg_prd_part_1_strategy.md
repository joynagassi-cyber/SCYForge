# PRD MindForge — Version 2.0 Complète & Consolidée
...
**Auteur** : Architecture Team MindForge  
...
### 1.1 Vision en 3 Phrases

**MindForge** est la première plateforme d'apprentissage entièrement autonome : l'utilisateur déclare un objectif de compétence, et 13 agents IA orchestrent automatiquement l'ingestion de contenu, la génération de parcours, les révisions espacées, l'assistance contextuelle et la certification — sans que l'utilisateur ait à choisir quelle feature utiliser.
...
**Value Proposition** : *"Tu déclares un objectif → MindForge s'occupe du reste jusqu'à ce que tu sois compétent, vérifiablement."*

### 1.2 Différenciation Clé v2.4

| Aspect | Anki | Notion | Coursera | MindForge v1 | **MindForge v2.4** |
|--------|------|--------|----------|-------------|-----------------|
...
**Problème additionnel résolu par MindForge v2 :**
...
### 2.2 Solution MindForge v2 — Architecture Unifiée
...
Pour asseoir sa rentabilité et conquérir le marché des entreprises, MindForge se positionne comme l'unique plateforme de formation professionnelle axée sur le **Retour sur Investissement (ROI) de la montée en compétences** prouvé par les données (SMI). Les détails complets du plan d'expansion, de la console manager, de l'onboarding sécurisé et de l'analyse financière lean sont documentés dans le guide **`uploads/mindforge_b2b_expansion_strategy.md`** :
...
**Objectif** : Documenter les 20 patterns architecturaux critiques qui garantissent la fiabilité, la scalabilité, la maintenabilité et la sécurité de MindForge — au-delà des patterns déjà couverts (§15).
...
**Intégration** :
- Tout `MindForgeEvent` qui échoue 3 fois → DLQ
...
CREATE INDEX idx_outbox_pending ON mfg_outbox(status, created_at)
...
async fn complete_node(ctx: &Context, node_id: Uuid, smi: f32) -> Result<()> {
    ...
    let event = MindForgeEvent::NodeCompleted { user_id: ctx.user_id, node_id, smi_achieved: smi };
    sqlx::query("INSERT INTO mfg_outbox (id, aggregate_type, aggregate_id, event_type, event_payload, status) VALUES ($1, $2, $3, $4, $5, 'pending')")
...
async fn outbox_publisher(ctx: Arc<Context>) {
    loop {
        let events = sqlx::query_as::<_, OutboxRecord>(
            "SELECT * FROM mfg_outbox WHERE status = 'pending' LIMIT 50"
...
| Principe | Implémentation MindForge |
|----------|--------------------------|
...
```rust
// ACL : Traduit modèles externes (APIs, formats) → modèles domaine MindForge
...
SELECT * FROM mfg_concepts FOR SYSTEM_TIME AS OF '2026-01-15 14:30:00'
WHERE user_id = $1;

-- "Quand ce paramètre FSRS a-t-il changé ?"
SELECT * FROM mfg_apex_cards
...
SELECT * FROM mfg_concepts FOR SYSTEM_TIME AS OF NOW() - INTERVAL '1 day'
...
La Pipeline Agentique ASCENT est la **feature centrale de MindForge v2**. Elle transforme MindForge d'une collection de features excellentes en un **organisme vivant** dont le seul but est de faire atteindre la compétence à l'utilisateur.

```
AVANT (MindForge v1)            APRÈS (MindForge v2)
─────────────────────           ──────────────────────────────────
