...
Pour maximiser la rentabilité de SCY Forge et offrir aux créateurs de contenu et aux plateformes tierces la possibilité de s'intégrer nativement dans notre écosystème d'apprentissage, SCY Forge expose ses moteurs sémantiques sous forme d'**APIs cloud-native sécurisées et facturées à l'usage (SaaS API)**.
...
Chaque requête API est vérifiée en amont par notre proxy **LiteLLM**. Les quotas et clés d'accès sont gérés de façon asynchrone, consignant l'usage dans la table `scy_llm_spend_log` pour s'assurer qu'un utilisateur ne dépasse jamais son enveloppe.
